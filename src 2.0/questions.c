...
extern void display_categories(){
    ...
    for (int i = 0; i < NUM_CATEGORIES; i++){
        ...
        for (int i = 0; i <= NUM_QUESTIONS/4; i++){
            ...
            printf( BLUE "|%11i", questions[i].value);
            printf( BLUE "|%11i", questions[i].value);
            printf( BLUE "|%11i|", questions[i].value);
...
void display_question(char *category, int value){
    ...
    if(strcmp(category, categories[0]) == 0){
        printf( YELLOW "%s: ", questions[(value - 100)/100].question);
    }
    else if(strcmp(category, categories[1]) == 0){
        printf( YELLOW "%s: ", questions[(value + 300)/100].question);
    }
    else if(strcmp(category, categories[2]) == 0){
        printf( YELLOW "%s: ", questions[(value + 700)/100].question);
    }
